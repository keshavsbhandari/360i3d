...
from convfused import ConvFused
...
class Convfused_CNN():
    def __init__(self, nb_epochs, lr, batch_size, resume, start_epoch, evaluate, train_loader, val_loader, channel,
                 test_loader):
    ...
    def getMCNN(self):
        ...
        self.MCNN = MCNN
        if torch.cuda.device_count()>1:
            print("DATA PARALLELISM USED : MCNN")
            self.MCNN.model = torch.nn.DataParallel(self.MCNN.model)
    ...
    def getSCNN(self):
        ...
        if torch.cuda.device_count()>1:
            print("DATA PARALLELISM USED : SCNN")
            self.SCNN.model = torch.nn.DataParallel(self.SCNN.model)

    def fusedCNN(self,d1,d2):
        _,(_,mcnnout) = self.MCNN.extractProbability(d1.cuda())
        _,(_,scnnout) = self.SCNN.extractProbability(d2.cuda())
    ...
    def build_model(self):
        ...
        # build model
        self.model = ConvFused().cuda()
        # print self.model
        if torch.cuda.device_count()>1:
            print("DATA PARALLELISM USED : CONVFUSED")
    ...
    def run(self):
        ...
        for self.epoch in range(self.start_epoch, self.nb_epochs):
            ...
            if is_best:
                ...
                with open('record_partial/convfused/best_motion_video_preds.pickle', 'wb') as f:
            ...
            }, is_best, 'record_partial/convfused/checkpoint.pth.tar', 'record_partial/convfused/model_best.pth.tar')
    ...
    def train_1epoch(self):
        ...
        for i, ((d1,d2), label_dict) in enumerate(progress):
            # measure data loading time

            data = self.fusedCNN(d1, d2)
        ...
        record_info(info, 'record_partial/convfused/new_opf_train.csv', 'train')
    ...
    def validate_1epoch(self, test=False):
        ...
        with torch.no_grad():
            for i, ((d1,d2), label_dict) in enumerate(progress):
                ...
                c_ = labellist_reverse.get(predicted_label)
                s_ = c_.split('/')[0]
        ...
        pickleme('record_partial/convfused/' + f + 'latest_motion_dic_video_level_preds.pickle', self.dic_video_level_preds)
        pickleme('record_partial/convfused/' + f + 'latest_motion_dic_super_class_label_preds.pickle',
                 self.dic_super_class_label_preds)
        pickleme('record_partial/convfused/' + f + 'latest_motion_dic_sub_class_label_preds.pickle',
                 self.dic_sub_class_label_preds)

        record_info(info, 'record_partial/convfused/' + f + 'new_opf_test.csv', 'test')

        from pandas_ml import ConfusionMatrix

        confusion_matrix_super = ConfusionMatrix(true_super, pred_super)
        df_conf_super = confusion_matrix_super.to_dataframe()
        df_conf_super.to_csv('./record_partial/convfused/super_confusion.csv', index=None)
        ...
        df_conf_sub.to_csv('./record_partial/convfused/sub_confusion.csv', index=None)
