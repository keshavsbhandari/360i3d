from i3d import I3D
from i3dloader import EgoCentricI3dLoader
from dataloader import EgoCentricDataLoader
import numpy as np
import pickle
...
from datautils import openfile
...
parser = argparse.ArgumentParser(description='EgoCentric 360 DataSet')
parser.add_argument('--epochs', default=50, type=int, metavar='N', help='number of total epochs')
parser.add_argument('--batch-size',default=50, type=int, metavar='N', help='mini-batch size (default: 64)')
...
parser.add_argument('--depth', dest='depth', default=100, type=int)
parser.add_argument('--trained-flowpath', dest='trainedflow', default='model/model_flow.pth', type=str, metavar='PATH',
                    help='path to initial flow weight loads')
parser.add_argument('--trained-rgbpath', dest='trainedrgb', default='model/model_rgb.pth', type=str, metavar='PATH',
                    help='path to initial rgb weight loads')
...
def data_parallel(module, input, device_ids=device_ids, output_device=None):
    replicas = torch.nn.parallel.replicate(module, device_ids)
    inputs = nn.parallel.scatter(input, device_ids)
    outputs = nn.parallel.parallel_apply(replicas, inputs)
    result = nn.parallel.gather(outputs, device_ids[0])
    return result
...
def main():
    ...
    model = FLOW_Inception(
        ...
        # pretrained_weight = arg.trainedflow
...
class FLOW_Inception():
    def __init__(self, nb_epochs, lr, batch_size, resume, start_epoch, evaluate, train_loader, test_loader,val_loader,
        ...
        self.child_r = labellist_reverse
        self.parent_r = parent_reverse
        self.num_classes = len(list(self.child_r.keys()))
        self.pretrained_weight = pretrained_weight
        self.BEST_SAVED_COUNT = 0
        self.model_resumed = False
    ...
    def build_model(self):
        ...
        self.model = I3D(num_classes=self.num_classes, modality='flow').cuda()
        self.model = torch.nn.DataParallel(self.model)
        # self.model.to(device)
        # if torch.cuda.device_count() > 1:
        #     print("USING MULTI GPUS")
        #     self.model = nn.DataParallel(self.model, device_ids=1,2,3,4,5,6)

        if self.pretrained_weight:
            self.load_pre_trained(self.pretrained_weight)
    ...
    def extractProbability(self, data):
        if not self.model_resumed:
            self.build_model()
            ...
            self.model.eval()
        with torch.no_grad():
            output = self.model(Variable(data.cuda()).cuda())
        return output
    ...
    def run(self):
        ...
        for self.epoch in range(self.start_epoch, self.nb_epochs):
            ...
            if is_best:
                self.best_prec1 = acc
                self.BEST_SAVED_COUNT += 1
                with open('./record_partial/flow/best_flow_preds.pickle', 'wb') as f:
            ...
            }, is_best, './record_partial/flow/checkpoint.pth.tar', 'record_partial/flow/model_best.pth.tar')
    ...
    def train_1epoch(self):
        ...
        for i, (data,label_dict) in enumerate(progress):
            ...
            tvar = torch.eye(63)[label_dict['label']]
            tvar = tvar.float().cuda()
            tvar = Variable(tvar)
            ...
            # output, _ = data_parallel(self.model, input_var, device_ids=[2,3,4,5,6],output_device=None)

            # print(output.data.shape)
            # loss = self.criterion(output, tvar)
        ...
        record_info(info, './record_partial/flow/new_opf_train.csv', 'train')
    ...
    def validate_1epoch(self, test = False):
        ...
        with torch.no_grad():
            for i, (batch,label_dict) in enumerate(progress):
                ...
                # compute output
                output, _ = self.model(data_var)

                tvar = torch.eye(63)[label_dict['label']]
                tvar = tvar.float().cuda()

                # val_loss.update(self.criterion(output, tvar))
                ...
                c_ = self.child_r.get(predicted_label)
                s_ = c_.split('/')[0]
        ...
        pickleme('./record_partial/flow/' + f + 'latest_motion_dic_video_level_preds.pickle', self.dic_video_level_preds)
        pickleme('./record_partial/flow/' + f + 'latest_motion_dic_super_class_label_preds.pickle',
                 self.dic_super_class_label_preds)
        pickleme('./record_partial/flow/' + f + 'latest_motion_dic_sub_class_label_preds.pickle',
                 self.dic_sub_class_label_preds)

        record_info(info, './record_partial/flow/' + f + 'new_opf_test.csv', 'test')

        from pandas_ml import ConfusionMatrix

        confusion_matrix_super = ConfusionMatrix(true_super, pred_super)
        df_conf_super = confusion_matrix_super.to_dataframe()
        df_conf_super.to_csv('./record_partial/flow/super_confusion.csv', index=None)
        ...
        df_conf_sub.to_csv('./record_partial/flow/sub_confusion.csv', index=None)
        ...
        print("=" * 20)
        print("LAST BEST SAVED COUNT : ", self.BEST_SAVED_COUNT, "  EPOCH : ", self.epoch)
        print("=" * 20)
