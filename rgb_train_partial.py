...
class RGB_Inception():
    ...
    def build_model(self):
        ...
        self.model = I3D(num_classes=self.num_classes, modality='rgb').cuda()
    ...
    def run(self):
        ...
        for self.epoch in range(self.start_epoch, self.nb_epochs):
            ...
            if is_best:
                ...
                self.LAST_BEST_EPOCH = self.epoch
                with open('./record_partial/rgb/best_flow_preds.pickle', 'wb') as f:
            ...
            }, is_best, './record_partial/rgb/checkpoint.pth.tar', 'record_partial/rgb/model_best.pth.tar')
    ...
    def train_1epoch(self):
        ...
        record_info(info, './record_partial/rgb/new_opf_train.csv', 'train')
    ...
    def validate_1epoch(self, test = False):
        ...
        pickleme('./record_partial/rgb/' + f + 'latest_motion_dic_video_level_preds.pickle', self.dic_video_level_preds)
        pickleme('./record_partial/rgb/' + f + 'latest_motion_dic_super_class_label_preds.pickle',
                 self.dic_super_class_label_preds)
        pickleme('./record_partial/rgb/' + f + 'latest_motion_dic_sub_class_label_preds.pickle',
                 self.dic_sub_class_label_preds)

        record_info(info, './record_partial/rgb/' + f + 'new_opf_test.csv', 'test')

        from pandas_ml import ConfusionMatrix

        confusion_matrix_super = ConfusionMatrix(true_super, pred_super)
        df_conf_super = confusion_matrix_super.to_dataframe()
        df_conf_super.to_csv('./record_partial/rgb/super_confusion.csv', index=None)
        ...
        df_conf_sub.to_csv('./record_partial/rgb/sub_confusion.csv', index=None)
        ...
        print("LAST BEST EPOCH : ",self.LAST_BEST_EPOCH," BEST PREC ",self.best_prec1)
        print("=" * 20)
