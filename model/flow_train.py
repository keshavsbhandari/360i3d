from i3d import I3D
from i3dloader import EgoCentricI3dLoader as dataloader
...
parser.add_argument('--trained-flowpath', dest = 'trainedflow', default='model/model_flow.pth',type=str, metavar='PATH', help = 'path to initial flow weight loads')
parser.add_argument('--trained-rgbpath', dest = 'trainedrgb', default='model/model_rgb.pth',type=str, metavar='PATH', help = 'path to initial rgb weight loads')
...
class FLOW_Inception():
    def __init__(self, nb_epochs, lr, batch_size, resume, start_epoch, evaluate, train_loader, channel,test_loader,mapper_path = './data_list/mapper',pretrained_weight = None):
        ...
        self.child_r = openfile(join(mapper_path, 'label_to_sub-class.txt'))
        self.parent_r = openfile(join(mapper_path, 'label_to_super-class.txt'))
    ...
    def build_model(self):
        ...
        self.optimizer = torch.optim.SGD(i3nception_pt.parameters(), lr=0.001, momentum=0.9)
    ...
    def run(self):
        ...
        for self.epoch in range(self.start_epoch, self.nb_epochs):
            ...
            if is_best:
                self.best_prec1 = acc
                with open('record/flow/best_flow_preds.pickle','wb') as f:
            ...
            }, is_best, 'record/flow/checkpoint.pth.tar', 'record/flow/model_best.pth.tar')

            self.validate_1epoch(test=True)

    def train1_epoch(self):
        ...
        for i, label_dict in enumerate(progress):
            # measure data loading time
            data = label_dict['x']

            data_time.update(time.time() - end)

            label = label_dict['label_c'].cuda()
            ...
            prec1, prec3, prec5 = kaccuracy_ontraining(output, label_dict['label_c'])
        ...
        record_info(info, 'record/flow/new_opf_train.csv', 'train')
    ...
    def validate_1epoch(self):
        ...
        with torch.no_grad():
            for i, label_dict in enumerate(progress):
                ...
                c = label_dict['child'][0]
                s = label_dict['parent'][0]

                match1 = topk(output, label_dict['label_c'], 1)
                match3 = topk(output, label_dict['label_c'], 3)
                match5 = topk(output, label_dict['label_c'], 5)

                predicted_label = torch.max(output, 1)[1].item()
                # Predicted superclass s_, and subclass c_
                c_ = self.child_r.get(str(predicted_label))
                s_ = c_.split('/')[0]
        ...
        pickleme('record/flow/' + f + 'latest_motion_dic_video_level_preds.pickle', self.dic_video_level_preds)
        pickleme('record/flow/' + f + 'latest_motion_dic_super_class_label_preds.pickle',
                 self.dic_super_class_label_preds)
        pickleme('record/flow/' + f + 'latest_motion_dic_sub_class_label_preds.pickle',
                 self.dic_sub_class_label_preds)

        record_info(info, 'record/flow/' + f + 'new_opf_test.csv', 'test')

        from pandas_ml import ConfusionMatrix

        confusion_matrix_super = ConfusionMatrix(true_super, pred_super)
        df_conf_super = confusion_matrix_super.to_dataframe()
        df_conf_super.to_csv('./record/flow/super_confusion.csv', index=None)
        ...
        df_conf_sub.to_csv('./record/flow/sub_confusion.csv', index=None)
